...
export default function Users({ navigation, drawer }) {
  ...
  useEffect(() => {
    ...
    } else if(filteringSection == "قسم الأيتام") {
      setUsersList(
        userList.filter(
          (info) => info[2] == filteringSection || info[2] == "وسيط اجتماعي"
        )
      );
    }else if(filteringSection == "قسم القفة"){
      setUsersList(
        userList.filter(
          (info) => info[2] == filteringSection || info[2] == "موزع القفة"
        )
      );
    }
    else {
  ...
  const filterInformations = (section) => {
    ...
    } else if (section == "قسم القفة") {
      setUsersList(
        userList.filter(
          (info) => info[2] == section || info[2] == "موزع القفة"
        )
      );
    }
